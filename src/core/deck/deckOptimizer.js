const { calculateSynergy } = require('../../analyses/synergy/synergyCalculator');
const { identifyCardMechanics, canIncludeWithAvatar, evaluateRegionalStrategy } = require('../cards/cardAnalysis');
const { analyzeElementalRequirements, calculateElementalDeficitContribution } = require('../../analyses/position/elementRequirementAnalyzer');

function getMaxCopiesForRarity(rarity) {
    rarity = (rarity || "").toLowerCase();
    if (rarity.includes("unique")) return 1;
    if (rarity.includes("elite")) return 2;
    if (rarity.includes("exceptional")) return 3;
    return 4; // Default for ordinary cards
}

function addCardWithCopies(card, deck, maxCardsToAdd, copiesInDeck, selectedAvatar) {
...
function enforceRarityLimits(cards) {
    const result = [];
    const countByName = {};
    ...
    for (const card of cards) {
        ...
        // Determine maximum allowed copies based on rarity
        let maxCopies = 4; // Default for ordinary cards
        if (rarity.includes("unique")) {
            maxCopies = 1;
        } else if (rarity.includes("elite")) {
            maxCopies = 2;
        } else if (rarity.includes("exceptional")) {
            maxCopies = 3;
        }
...
function optimizeDeck(selectedSpells, minions, artifacts, auras, magics) {
    ...
    // Current mana curve
    const manaCosts = selectedSpells.map(card => card.mana_cost);
    const currentCurve = {};
    ...
    for (const [cost, minCount] of Object.entries(criticalManaCosts)) {
        const currentCount = currentCurve[cost] || 0;
    ...
    for (const [cost, adjustment] of Object.entries(adjustments)) {
        if (adjustment < 0) {
            ...
            const cardsOfCost = optimizedSpells
                ...
                .filter(item => Math.min(item.card.mana_cost, 7) === adjustedCost);
            ...
            if (cardsOfCost.length > 0) {
                ...
                const cardsToRemove = cardsOfCost
                    .sort((a, b) => {
                        ...
                        const aIsCombo = combos.some(combo => 
                            combo.pieces.some(piece => piece.card.baseName === a.card.baseName));
                        const bIsCombo = combos.some(combo => 
                            combo.pieces.some(piece => piece.card.baseName === b.card.baseName));
                        ...
                        const rarityWeight = {unique: 4, elite: 3, exceptional: 2, ordinary: 1};
    ...
    for (const [cost, adjustment] of priorityCosts) {
        ...
        let candidates = allCandidates
            .filter(card => Math.min(card.mana_cost, 7) === adjustedCost &&
                !optimizedSpells.some(c => c.baseName === card.baseName) &&
        ...
        if (candidates.length < adjustment && parseInt(cost) <= 2) {
            ...
            const flexibleCandidates = allCandidates
                .filter(card => {
                    ...
                    if (candidates.some(c => c.baseName === card.baseName)) return false;
                    
                    // Don't include cards we already have in the deck
                    if (optimizedSpells.some(c => c.baseName === card.baseName)) return false;
                    
                    // Don't include cards that are much more expensive
                    if (card.mana_cost > parseInt(cost) + 1) return false;
            ...
            candidates.sort((a, b) => {
                ...
                const aPowerValue = (a.power || 0) / Math.max(a.mana_cost, 1);
                const bPowerValue = (b.power || 0) / Math.max(b.mana_cost, 1);
            ...
            for (const candidate of candidates.slice(0, adjustment)) {
                const baseName = candidate.baseName;
                const rarity = (candidate.rarity || "").toLowerCase();

                // Count how many of this card we already have
                const currentCount = optimizedSpells.filter(c => c.baseName === baseName).length;
                const maxCopies = getMaxCopiesForRarity(rarity);
